...
<table width="100%" border="1" cellspacing="1" cellpadding="1">
  <tr>
	...
	<table width="100%"  border="0" cellspacing="1" cellpadding="0" bordercolor="#FFFFFF">
			...
			?>
			<select name="lstOpcion" class="CajaTexto" id="lstOpcion" onChange="activa()">
			<?php 
			while ($reg1=mssql_fetch_array($cursor1)) 
			{ ?>
			  <option value="<?php echo $reg1[codSubItem]; ?>"><?php echo $reg1[nomSubItem]; ?></option>
			<? } ?>
			</select></td>
		</tr>
   	...
   	<?php	 
		...
		while($datos_items=mssql_fetch_array($cursor))
		{
			$i++;
	?>
        <tr>			
			...
			<td class="TxtTabla"  ><input class="CajaTexto" type="text"  id="item<? echo $i; ?>" name="item<? echo $i; ?>"></td>
