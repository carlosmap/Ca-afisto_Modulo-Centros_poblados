...
if ($recarga == "2") 
{
	...
	if($accion==2)
	{
//echo "cis rurrrrrrrrrt ".$cod_rut."<br><br>";
...
{
//	alert (val);
	//obtiene el valor de el radio buton sobre el que se ha echo cick
/*
	for(i=0; i <document.form1.rut.length; i++)
	{
		if(document.form1.rut[i].checked)
		{
			valorSeleccionado = document.form1.rut[i].value;
		}
	}
alert(valorSeleccionado);
*/
	if(val=='no')//and(valorSeleccionado==0))
	{
...
//Si todas las validaciones fueron correctas, el formulario hace submit y permite grabar
	...
	//preguntar();
<?
if($accion!=3) //cuando la accion es eliminar, no valida la informacion
{
?>
...
?>
                  ...
                  <tr>
                    ...
                    <td class="TxtTabla">Si<input type="radio" name="rut" id="rut" value="1"  onClick="campRut('si')" <? if($hay_rut==1) { echo "checked"; } ?> <?php echo $disa; ?>> 
                      No<input type="radio" name="rut" id="rut" value="0"   onClick="campRut('no')"  <? if($hay_rut==0) { echo "checked"; } ?> <?php echo $disa; ?> ></td>
                  </tr>
                  <tr>
                    <td width="70%" class="TituloTabla1">6.8.4.4 Codigo RUT</td>
                    <td class="TxtTabla"><input type="text" name="cod_rut" id="cod_rut" value="<? echo $cod_ruts; ?>" <? if($hay_rut==0){ echo "disabled"; } ?> class="CajaTexto" onKeyPress="return acceptNum2(event)"  <?php echo $disa; ?>></td>
                  </tr>
                  <tr>
				  ...
				  <tr>
					...
					<table width="100%" border="0">
                      ...
                      <?php
						...
						$sql_acti="select * from tmItems where codOpcion=135";
						$sql_acti= $sql_acti. " and tmItems.codProyecto=".$_SESSION["ccfProyecto"] ;
						$sql_acti= $sql_acti. " AND tmItems.codModulo=".$_SESSION["ccfModulo"] ;
						$m=1;
						$cursor_activi=mssql_query($sql_acti);
...
?>						<input type="hidden" name="cMaquinas" id="cMaquinas" value="<?=$m ?>" />
                ...
                <table width="100%" border="0" cellSpacing="1" cellPadding="0" >
                  ...
                  <tr>
                    ...
                    <td class="TxtTabla"><select name="Umedida" class="CajaTexto" id="Umedida" <? echo $disa; ?> >
                      <?
...
?>
                  ...
                  <tr>
                    ...
                    <td class="TxtTabla"><select name="ventas" class="CajaTexto" id="ventas" <?php echo $disa; ?>>
                      <option value=""></option>
                      <?php
						$sql_acti="select * from tmItems where codOpcion=96";
						$sql_acti= $sql_acti. " and tmItems.codProyecto=".$_SESSION["ccfProyecto"] ;
						$sql_acti= $sql_acti. " AND tmItems.codModulo=".$_SESSION["ccfModulo"] ;
						$cursor_activi=mssql_query($sql_acti);
						while($datos_activi=mssql_fetch_array($cursor_activi))
