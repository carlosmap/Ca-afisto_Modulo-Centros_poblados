...
//echo " regsitros 2.1 <br>".$sqlRta." -- ".mssql_get_last_message()."<br><br>";	
			...
			$cant_reg=mssql_num_rows($cursorRta); //si no se encontraron registros, es por que no existe informacion relacionada con los items de la pregunta
			...
			if($cant_reg==0)
			{
				?>
				<td align="center">
				<input name="<? echo $j; ?>aplicaResp<? echo $i; ?>" type="checkbox" id="<? echo $j; ?>aplicaResp<? echo $i; ?>" value="1"  <? echo $dis; ?>>
				<input name="<? echo $j; ?>codResp<? echo $i; ?>" type="hidden" id="<? echo $j; ?>codResp<? echo $i; ?>" value="<? echo $reg2[codSubItem]; ?>">			</td>
				<?
				$j++;
			}
