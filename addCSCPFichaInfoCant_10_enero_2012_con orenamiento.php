...
//funcion que valida los campos de las preguntas de ordenamiento
function valida_campo_orden()
{
	//almacenamos la cantidad de registros(filas) generados en el formulario
	var pCantItems=1*document.form1.cantidadItem.value;
...
//						ban_vacio="si";
				...
				if((<? echo $Sum; ?><document.getElementById(cantidad).value) )
				{
//						ban_vacio="si";
						alert("El valor ingresado no puede ser superior a  "+<? echo $Sum; ?>);
						ret=false;
						break;									
				}
			
				valores[i]=document.getElementById(cantidad).value;
	}

	var cant = valores.length;
	var cant_encontrados=0;

	for(var i= 1; i <=pCantItems; i++) 
	{

		for(z=1;z<=<? echo $Sum; ?>;z++)
		{
			if(valores[i]==z)
				cant_encontrados++;
			alert(cant_encontrados+" "+z+" "+valores[i]);
			if(cant_encontrados==2)
			{
				alert("No se pueden repetir las respuestas");
				break;
				break;	
				ret=false;			
			}	
...
//Si todas las validaciones fueron correctas, el formulario hace submit y permite grabar
	if ((v1=='s') && (v2=='s')  && (v3=='s')) 
	{
		//valida los campos, utilizado para las preguntas de ordenamiento
